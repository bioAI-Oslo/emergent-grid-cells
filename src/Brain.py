...
class Brain:
    def __init__(self, env, npcs, sigma=0.12, dtype=torch.float32):
        """
        Args:
    ...
    def softmax_response(self, pos, DoG=False, surround_scale=2, p=2.0, compute_mode="use_mm_for_euclid_dist_if_necessary"):
        ...
        dists = torch.cdist(pos, self.pcs, p=p, compute_mode=compute_mode)
        ...
        if DoG:
            ...
            activity -= torch.min(activity, dim=-1, keepdim=True)
